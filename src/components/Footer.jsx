...
function Footer() {
  ...
  return (
    <motion.div data-scroll data-scroll-scetion data-scroll-speed="-.3" className='w-full h-screen flex gap-5 p-20 text-black font-["Fonders_Grotesk"]'>
      <div className='w-1/2 h-full flex flex-col justify-between '>
        <div className='heading'>
        <h1 className='text-[7vw] font-semibold uppercase leading-none -md-10'>eye-</h1>
        <h1 className='text-[7vw] font-semibold uppercase leading-none -md-10'>opening</h1>
        </div>
      ...
      <div className='w-1/2'>
      <h1 className='text-[5.5vw] font-semibold uppercase leading-none -md-10 '>Presenatation</h1>
      <h1 className='text-xl'>S:</h1>
      <div className='dets'>
        {["Instagram","Behance","Facebook","Linkedin"].map((item,index)=>(
          <a key={index} className={`text-lg cursor-pointer font-light block capitalize ${index===4 && "ml-44"} `}>{item}</a>
        ))}
      </div>
      <h1 className='mt-10 text-xl'>L:</h1>
      
      <div className='alldets flex justify-between'>
      <div className='dets2 mt-5'>
          <h1 className='text-xl hover:underline'>202-1965 W 4th <br />AveVancouver, Canada</h1>
          <h1 className='text-xl hover:underline mt-5'>230 Chukarina St <br />Lviv, Ukraine</h1>
      </div>
      <div className='dets3 '>
          <h1 className='text-xl mb-5'>M:</h1>
          {["Service","Our Work","About us","Insights","contact"].map((item,index)=>(
          <a key={index} className={`text-lg cursor-pointer hover:underline font-light block capitalize ${index===5 && "ml-44"} `}>{item}</a>
        ))}
      </div>
      </div>
      </div>
